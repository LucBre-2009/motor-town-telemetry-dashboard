...
class App:
    ...
    def settings(self):
        ...
        self.label(
            win, "Dashboard and telemetry connection", 10, MUTED, bg=PANEL
        ...
        row = tk.Frame(appearance, bg=PANEL2)
        row.pack(fill="x", padx=18, pady=6)
        self.label(row, "Accent color", 11, MUTED, True, bg=PANEL2).pack(side="left")

        var = tk.StringVar(value=self.theme_name)
        menu = tk.OptionMenu(row, var, *THEMES.keys())
        menu.configure(bg=BG, fg=TEXT, activebackground=self.accent,
